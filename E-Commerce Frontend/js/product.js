...
function initializeRoleBasedUI() {
    const adminFormSection = document.getElementById('admin-form-section');
    const wishlistNavLink = document.getElementById('wishlist-nav');
    const cartNavLink = document.getElementById('cart-nav');
    
    // Get fresh values from localStorage
    const currentRole = localStorage.getItem('userMode');
    const currentUserId = localStorage.getItem('userId');

    console.log("Current State:", { currentRole, currentUserId });

    // 1. Handle Admin Section
    if (adminFormSection) {
        adminFormSection.style.display = (currentRole === 'admin') ? 'block' : 'none';
    }

    // 2. Handle Navbar Links (Wishlist & Cart)
    if (wishlistNavLink || cartNavLink) {
        // Only show Wishlist/Cart if someone is actually logged in
        if (currentUserId) {
            if (wishlistNavLink) wishlistNavLink.style.display = 'block';
            if (cartNavLink) cartNavLink.style.display = 'block';
        } else {
            // Hide for guests (not logged in)
            if (wishlistNavLink) wishlistNavLink.style.display = 'none';
...
<div class="card product-card h-100">
    ...
    <div class="card-body p-3">
        ...
        ${window.generateStarRating ? window.generateStarRating(product.averageRating, product.totalReviews) : ''}
    ...
    try {
        ...
        if (response.ok) {
            ...
            const mappedProducts = backendProducts.map(product => ({
                ...
                // 🔥 ADDED THESE TWO LINES SO THE FRONTEND SEES THE MATH:
